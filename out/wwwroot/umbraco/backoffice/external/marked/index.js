...
function u(c, e = "") {
  ...
  const n = {
    replace: (s, i) => {
      ...
      return r = r.replace(b.caret, "$1"), t = t.replace(s, r), n;
    },
...
const b = {
  ...
  lheading: we,
  table: Y,
  paragraph: u(N).replace("hr", C).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", Y).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", q).getRegex()
}, Ae = {
  ...Q,
  ...
  ...M,
  br: u(ae).replace("{2,}", "*").getRegex(),
  text: u(M.text).replace("\\b_", "\\b_| {2,}\\n").replace(/\{2,\}/g, "*").getRegex()
}, I = {
  normal: Q,
  gfm: ze,
  pedantic: Ae
...
function R(c, e) {
  if (e) {
    if (b.escapeTest.test(c))
      return c.replace(b.escapeReplace, ee);
  } else if (b.escapeTestNoEncode.test(c))
    return c.replace(b.escapeReplaceNoEncode, ee);
  return c;
}
function te(c) {
  try {
    c = encodeURI(c).replace(b.percentDecode, "%");
  } catch {
    return null;
  }
  return c;
...
function ne(c, e) {
  const t = c.replace(b.findPipe, (i, r, l) => {
    let o = !1, a = r;
    for (; --a >= 0 && l[a] === "\\"; )
      o = !o;
    return o ? "|" : " |";
...
function se(c, e, t, n, s) {
  const i = e.href, r = e.title || null, l = c[1].replace(s.other.outputLinkReplace, "$1");
  if (c[0].charAt(0) !== "!") {
    n.state.inLink = !0;
    const o = {
      type: "link",
...
function Je(c, e, t) {
  const n = c.match(t.other.indentCodeCompensation);
  if (n === null)
    return e;
  const s = n[1];
  return e.split(`
...
`)
  ...
  fences(e) {
    ...
    if (t) {
      const n = t[0], s = Je(n, t[3] || "", this.rules);
      return {
        type: "code",
        raw: n,
...
` + n.join(`
`), m = this.blockquote(k);
          r[r.length - 1] = m, s = s.substring(0, s.length - x.raw.length) + m.raw, i = i.substring(0, i.length - x.text.length) + m.text;
          break;
        } else if (f?.type === "list") {
          const x = f, k = x.raw + `
` + n.join(`
`), m = this.list(k);
          r[r.length - 1] = m, s = s.substring(0, s.length - f.raw.length) + m.raw, i = i.substring(0, i.length - x.raw.length) + m.raw, n = k.substring(r.at(-1).raw.length).split(`
`);
          continue;
        }
      }
...
`, 1)[0], x = !d.trim(), k = 0;
        if (this.options.pedantic ? (k = 2, p = d.trimStart()) : x ? k = t[1].length + 1 : (k = t[2].search(this.rules.other.nonSpaceChar), k = k > 4 ? 1 : k, p = d.slice(k), k += t[1].length), x && this.rules.other.blankLine.test(f) && (h += f + `
`, e = e.substring(f.length + 1), a = !0), !a) {
          const Z = this.rules.other.nextBulletRegex(k), J = this.rules.other.hrRegex(k), K = this.rules.other.fencesBeginRegex(k), V = this.rules.other.headingBeginRegex(k), ke = this.rules.other.htmlBeginRegex(k);
          for (; e; ) {
            const D = e.split(`
`, 1)[0];
            let T;
            if (f = D, this.options.pedantic ? (f = f.replace(this.rules.other.listReplaceNesting, "  "), T = f) : T = f.replace(this.rules.other.tabCharGlobal, "    "), K.test(f) || V.test(f) || ke.test(f) || Z.test(f) || J.test(f))
              break;
            if (T.search(this.rules.other.nonSpaceChar) >= k || !f.trim())
              p += `
` + T.slice(k);
            else {
              if (x || d.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4 || K.test(d) || V.test(d) || J.test(d))
                break;
              p += `
` + f;
            }
            !x && !f.trim() && (x = !0), h += D + `
`, e = e.substring(D.length + 1), d = T.slice(k);
          }
        }
        i.loose || (l ? i.loose = !0 : this.rules.other.doubleBlankLine.test(h) && (l = !0));
        let m = null, W;
        this.options.gfm && (m = this.rules.other.listIsTask.exec(p), m && (W = m[0] !== "[ ] ", p = p.replace(this.rules.other.listReplaceTask, ""))), i.items.push({
          type: "list_item",
          raw: h,
          task: !!m,
          checked: W,
      ...
      for (let a = 0; a < i.items.length; a++)
        if (this.lexer.state.top = !1, i.items[a].tokens = this.lexer.blockTokens(i.items[a].text, []), !i.loose) {
          const h = i.items[a].tokens.filter((d) => d.type === "space"), p = h.length > 0 && h.some((d) => this.rules.other.anyLine.test(d.raw));
          i.loose = p;
        }
      if (i.loose)
        for (let a = 0; a < i.items.length; a++)
  ...
  table(e) {
    ...
    if (!t || !this.rules.other.tableDelimiter.test(t[2]))
      return;
    const n = ne(t[1]), s = t[2].replace(this.rules.other.tableAlignChars, "").split("|"), i = t[3]?.trim() ? t[3].replace(this.rules.other.tableRowBlankLine, "").split(`
`) : [], r = {
      type: "table",
      raw: t[0],
      header: [],
    ...
    if (n.length === s.length) {
      ...
      for (const l of i)
        r.rows.push(ne(l, r.header.length).map((o, a) => ({
          text: o,
...
` ? t[1].slice(0, -1) : t[1];
  ...
  link(e) {
    ...
    if (t) {
      ...
      if (this.options.pedantic) {
        ...
        i = t[3] ? t[3].slice(1, -1) : "";
      return s = s.trim(), this.rules.other.startAngleBracket.test(s) && (this.options.pedantic && !this.rules.other.endAngleBracket.test(n) ? s = s.slice(1) : s = s.slice(1, -1)), se(t, {
  ...
  reflink(e, t) {
    ...
    if ((n = this.rules.inline.reflink.exec(e)) || (n = this.rules.inline.nolink.exec(e))) {
      ...
      return se(n, i, n[0], this.lexer, this.rules);
    }
  }
  emStrong(e, t, n = "") {
    ...
    if (!(s[1] || s[2] || "") || !n || this.rules.inline.punctuation.exec(n)) {
      ...
      for (p.lastIndex = 0, t = t.slice(-1 * e.length + r); (s = p.exec(t)) != null; ) {
        ...
        if (a -= o, a > 0)
          continue;
        o = Math.min(o, o + a + h);
        const d = [...s[0]][0].length, f = e.slice(0, r + s.index + d + o);
        if (Math.min(r, o) % 2) {
          const k = f.slice(1, -1);
          return {
            type: "em",
...
class w {
  ...
  inlineQueue;
  constructor(e) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = e || S, this.options.tokenizer = this.options.tokenizer || new B(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
      inLink: !1,
      inRawBlock: !1,
      top: !0
    };
    ...
    this.options.pedantic ? (t.block = I.pedantic, t.inline = z.pedantic) : this.options.gfm && (t.block = I.gfm, this.options.breaks ? t.inline = z.breaks : t.inline = z.gfm), this.tokenizer.rules = t;
  }
  /**
   * Expose Rules
   */
...
` + s.raw, this.inlineQueue.at(-1).src = r.text) : this.tokens.links[s.tag] || (this.tokens.links[s.tag] = {
      ...
      if (this.options.extensions?.startBlock) {
        ...
        this.options.extensions.startBlock.forEach((a) => {
          o = a.call({ lexer: this }, l), typeof o == "number" && o >= 0 && (r = Math.min(r, o));
...
` + s.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = r.text) : t.push(s);
  ...
  inlineTokens(e, t = []) {
    ...
    for (; e; ) {
      i || (r = ""), i = !1;
      let l;
      if (this.options.extensions?.inline?.some((a) => (l = a.call({ lexer: this }, e, t)) ? (e = e.substring(l.raw.length), t.push(l), !0) : !1))
        continue;
      ...
      if (l = this.tokenizer.reflink(e, this.tokens.links)) {
        ...
        const a = t.at(-1);
        l.type === "text" && a?.type === "text" ? (a.raw += l.raw, a.text += l.text) : t.push(l);
        continue;
      }
      if (l = this.tokenizer.emStrong(e, n, r)) {
        e = e.substring(l.raw.length), t.push(l);
        continue;
      }
      ...
      if (l = this.tokenizer.inlineText(o)) {
        e = e.substring(l.raw.length), l.raw.slice(-1) !== "_" && (r = l.raw.slice(-1)), i = !0;
        const a = t.at(-1);
        a?.type === "text" ? (a.raw += l.raw, a.text += l.text) : t.push(l);
        continue;
      }
      if (e) {
...
class E {
  ...
  code({ text: e, lang: t, escaped: n }) {
    const s = (t || "").match(b.notSpaceStart)?.[0], i = e.replace(b.endingNewline, "") + `
`;
...
`;
  ...
  link({ href: e, title: t, tokens: n }) {
    const s = this.parser.parseInline(n), i = te(e);
    if (i === null)
      return s;
    e = i;
    let r = '<a href="' + e + '"';
...
class y {
  ...
  textRenderer;
  constructor(e) {
    this.options = e || S, this.options.renderer = this.options.renderer || new E(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new X();
  }
  /**
   * Static Parse Method
   */
...
class Ke {
  ...
  use(...e) {
    ...
    return e.forEach((n) => {
      ...
      if (s.async = this.defaults.async || s.async || !1, n.extensions && (n.extensions.forEach((i) => {
        ...
        const i = this.defaults.renderer || new E(this.defaults);
        for (const r in n.renderer) {
          if (!(r in i))
            throw new Error(`renderer '${r}' does not exist`);
          if (["options", "parser"].includes(r))
            continue;
          const l = r, o = n.renderer[l], a = i[l];
          i[l] = (...h) => {
            let p = o.apply(i, h);
            return p === !1 && (p = a.apply(i, h)), p || "";
          };
        }
        s.renderer = i;
      }
      if (n.tokenizer) {
        ...
        for (const r in n.tokenizer) {
          ...
          if (["options", "rules", "lexer"].includes(r))
            continue;
          const l = r, o = n.tokenizer[l], a = i[l];
          i[l] = (...h) => {
            let p = o.apply(i, h);
            return p === !1 && (p = a.apply(i, h)), p;
          };
      ...
      if (n.hooks) {
        ...
        for (const r in n.hooks) {
          ...
          if (["options", "block"].includes(r))
            continue;
          const l = r, o = n.hooks[l], a = i[l];
          L.passThroughHooks.has(r) ? i[l] = (h) => {
            if (this.defaults.async)
              return Promise.resolve(o.call(i, h)).then((d) => a.call(i, d));
            const p = o.call(i, h);
            return a.call(i, p);
          } : i[l] = (...h) => {
            let p = o.apply(i, h);
  ...
  parseMarkdown(e) {
    return (n, s) => {
      ...
      const o = r.hooks ? r.hooks.provideLexer() : e ? w.lex : w.lexInline, a = r.hooks ? r.hooks.provideParser() : e ? y.parse : y.parseInline;
      if (r.async)
        return Promise.resolve(r.hooks ? r.hooks.preprocess(n) : n).then((h) => o(h, r)).then((h) => r.hooks ? r.hooks.processAllTokens(h) : h).then((h) => r.walkTokens ? Promise.all(this.walkTokens(h, r.walkTokens)).then(() => h) : h).then((h) => a(h, r)).then((h) => r.hooks ? r.hooks.postprocess(h) : h).catch(l);
      try {
...
Please report this to https://github.com/markedjs/marked.`, e) {
        const s = "<p>An error occurred:</p><pre>" + R(n.message + "", !0) + "</pre>";
...
g.options = g.setOptions = function(c) {
  return $.setOptions(c), g.defaults = $.defaults, re(g.defaults), g;
};
g.getDefaults = O;
g.defaults = S;
g.use = function(...c) {
  return $.use(...c), g.defaults = $.defaults, re(g.defaults), g;
};
g.walkTokens = function(c, e) {
...
g.Hooks = L;
g.parse = g;
const Ve = g.options, Ye = g.setOptions, et = g.use, tt = g.walkTokens, nt = g.parseInline, st = g, rt = y.parse, it = w.lex;
export {
  L as Hooks,
  w as Lexer,
  Ke as Marked,
